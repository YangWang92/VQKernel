...
>
void __global__ vq_attention_decoding_kernel(
    QKV_TYPE* _o,
    QKV_TYPE* _q, 
...
)
{
    ...
    const uint32_t thread_packed_load_width = PACKED_LOAD_WIDTH / (8 * sizeof(QKV_TYPE));
    using packed_load_type = typename packed_vec<QKV_TYPE, thread_packed_load_width>::Type;

    const uint32_t batch_per_block = min((uint32_t) 1, (uint32_t) (BLOCK_SIZE / TOKEN_BLOCK));
    ...
    const uint32_t tb_offset = blockIdx.y % BLOCK_PER_HEAD;             // block id inside all blocks process one head
    const uint32_t k_residuals = _k_cache_configs[2 * 32 + head_id];    // How many residual of k
    ...
    const uint32_t k_residual_id = tb_offset / k_residuals;             // 
    const uint32_t k_subspace_id = tb_offset % k_residuals;             // 
    const uint32_t k_compression_ratio = _k_cache_configs[0 * 32 + head_id];
    const uint32_t k_subspace_num = HEAD_DIM / k_compression_ratio;
    ...
    const uint32_t k_subspace_per_block_to_compute = max((uint32_t) 1, (uint32_t) ((k_subspace_num * k_residuals) / BLOCK_PER_HEAD));
    const uint32_t k_segment_element_num = k_subspace_per_block_to_compute * k_compression_ratio;

    // Load k codebook
    // Starting from codebook_offset, next k_subspace_per_block_to_compute rows.
    const uint32_t k_codebook_offset = _k_cache_configs[3 * 32 + head_id]
    ...
    const uint32_t codebook_to_access_k = k_subspace_per_block_to_compute * k_entry_num * k_compression_ratio; // codebook access begin of (residual id, subspace id)
    const uint32_t codebook_to_load_k = codebook_to_access_k / K_INV_HOT_ENTRY;
    ...
    uint32_t threads_need_to_load = min((uint32_t)BLOCK_SIZE, (uint32_t)(codebook_to_load_k / thread_packed_load_width)); // How many thread need to conduct the load
    uint32_t block_packed_load_width = threads_need_to_load * thread_packed_load_width;
    ...
    const float k_subspace_load_at_once = block_packed_load_width / (k_entry_to_load * k_compression_ratio);
    ...
    if (threadIdx.x < threads_need_to_load) {
        const uint32_t k_load_thread_group = k_entry_to_load * k_compression_ratio / thread_packed_load_width;
        const uint32_t group_id = threadIdx.x / k_load_thread_group;
        const uint32_t group_off = threadIdx.x % k_load_thread_group;
        for (int i = 0; i < k_load_iterations; i++) {
            ...
            *(packed_load_type*)(&_k_codebook[k_codebook_offset + 
                                              (uint32_t)(i * k_subspace_load_at_once) * k_subspace_stride + 
                                              group_id * k_subspace_stride + 
                                              group_off * thread_packed_load_width + 
                                              (i % k_group_needed_for_one_subspace) * block_packed_load_width
                                             ]);
        }
    }
    ...
    const float v_subspace_load_at_once = block_packed_load_width / (v_entry_to_load * k_compression_ratio);
    ...
    if (threadIdx.x < BATCH_SIZE) {
        // One thread load one batch's q and k
    ...
    QKV_TYPE* k_dequant_reg[token_per_thread][(B32REG_LIMIT / token_per_thread) * (4 / sizeof(QKV_TYPE))];
            ...
            while (_tid < TOKEN_BLOCK) {
                for (int _s = 0; _s < k_subspace_per_block_to_compute; _s++) {
                    ...
                    _k_cache[i * batch_per_block * _seq_len * _per_token_byte_k + 
                             ...
                             k_codeword_offset + 
                             (k_subspace_id * k_subspace_per_block_to_compute + _s) * k_residuals + 
                             k_residual_id
                            ];
                }
    ...
    uint32_t k_reg_iters = max((uint32_t) 1, (uint32_t)((k_subspace_per_block_to_compute * k_compression_ratio * sizeof(QKV_TYPE)) / (B32REG_LIMIT * sizeof(uint32_t))));
    uint32_t v_reg_iters = max((uint32_t) 1, (uint32_t)((v_subspace_per_block_to_compute * v_compression_ratio * sizeof(QKV_TYPE)) / (B32REG_LIMIT * sizeof(uint32_t))));
    QKV_TYPE* entry_addr;
    for (int i = 0; i < BATCH_SIZE / batch_per_block; i++) {
...
)
{
    ...
    if ((batch == 8) && (head_num == 32) && (head_dim == 128)) {
        auto kernel = vq_attention_decoding_kernel<8, WARP_NUM * WARP_SIZE, 32, 128, _TOKEN_BLOCK, _BLOCK_PER_HEAD, 1, 1, 1, 1, 128, 16, FALSE>;
