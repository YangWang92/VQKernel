...
)
{
    ...
    if (threadIdx.x < 256) {
        ...
        for (int k = 0; k < _k / K_BLOCK_TILE; k++) {
            ...
            // __syncthreads();
            // if (threadIdx.x == 0) printf("[Compute] (%d, %d) k = %d, Consuming H[%d] and D[%d]\n", blockIdx.x, blockIdx.y, k, k % 3, k % 2);
        ...
        for (int k = 0; k < _k / K_BLOCK_TILE; k++) {
            ...
            // __syncthreads();
            // D[k % 2] <- Dequant(W[k % 2])
            // if (threadIdx.x == 256) printf("[Dequant] (%d, %d) k = %d, Consuming W[%d], Producing D[%d]\n", blockIdx.x, blockIdx.y, k, k % 2, k % 2);
        ...
        for (int k = 0; k < _k / K_BLOCK_TILE; k++) { 
            ...
            // if (threadIdx.x == 512) printf("[   Load] (%d, %d) k = %d, Producing H[%d] and W[%d]\n", blockIdx.x, blockIdx.y, k, k % 3, k % 2);
            h_buf = &h_shmem[(k % 3) * M_BLOCK_TILE * K_BLOCK_TILE];
            w_buf = &w_shmem[(k % 2) * K_BLOCK_TILE * N_BLOCK_TILE / _compression_ratio];
...
int main(int argc, char** argv) {
    ...
    int m = 1024, n = 1024, k = 512, compression_ratio = 2;
    h_h = new half [m * k];
    h_o = new half [m * n];
